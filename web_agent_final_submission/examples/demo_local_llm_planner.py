...
from web_agent.models.hf_client import LocalHFModelClient
...
def run_mock_llm_task(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    trace_dir = make_unique_dir(
        Path(args.trace_root) / f"stage3b1_local_llm_{args.task_id}_{args.mock_mode}_{now_id()}"
    )
    ...
    planner = LLMPlanner(
        model_client = LocalHFModelClient(model_name=args.model, max_new_tokens=args.max_new_tokens),
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Stage 3B1 demo: local HF LLM planner + existing browser loop")
    parser.add_argument("--task-id", default="wiki_search_web_agent_results", help="Task id from tasks yaml")
    parser.add_argument("--model", default="Qwen/Qwen2.5-3B-Instruct", help="Local HuggingFace model name or path")
...
def main() -> None:
    ...
    print("\nStage 3B1 local LLM planner demo finished.")
