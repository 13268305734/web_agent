...
def normalize_action_dict(
    ...
    # Stage 3B1 parser hotfix:
    # 1. {"type": "click"} -> {"action": "click"}
    # 2. {"type": "finish", "value": "..."} -> {"action": "finish", "answer": "..."}
    # 3. {"action": {"action": "click", ...}, "element_id": 12} -> flatten nested action
    # 4. Ignore unrelated fields such as success_check.
    data = dict(data)
    ...
    if "action" not in data and "action_type" in data:
        data["action"] = data.get("action_type")

    if isinstance(data.get("action"), dict):
        outer = dict(data)
        nested = dict(data["action"])

        for key in [
            "thought", "element_id", "text", "value", "key",
            "seconds", "direction", "answer", "x", "y"
        ]:
            if key not in nested and key in outer:
    ...
    if "text" not in data and "value" in data:
        data["text"] = data.get("value")

    if (
        "answer" not in data
        and str(data.get("action", "")).strip().lower() == "finish"
        and "value" in data
    ):
        data["answer"] = data.get("value")
    ...
    # Some local LLMs output nested JSON like:
    # {"action": {"thought": "...", "action": "click", "element_id": 13}}
    # Flatten it before action validation.
    if isinstance(data.get("action"), dict):
        nested = data["action"]
        if "thought" not in nested and data.get("thought"):
            nested["thought"] = data.get("thought")
        data = nested
